...
export function TrendingIdeas({ onSelectIdea }: { onSelectIdea: (idea: TrendIdea) => void }) {
  ...
  // Mocked daily-at-midnight refresh. Backend integration point: replace this
  // client-side timer with a server-side cron/webhook — client timers don't
  // survive tab sleep or the tab being closed.
  useEffect(() => {
    ...
    <section className="rounded-[20px] bg-card p-6 shadow-[0_2px_8px_rgba(26,24,35,0.16)]">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-xs text-muted-foreground">Refreshes automatically every day at 12:00 AM</p>
        </div>
      </div>
